...
def get_information_work():
    ...
    channel_videos_combobox = customtkinter.CTkComboBox(channel_frame, values=video_watch_urls, width=ENTRY_WIDTH)
    channel_videos_combobox.grid(row=11, column=0, columnspan=2, padx=PADDING_X, pady=PADDING_Y, sticky="sw")
...
def start_download_work(audio_or_video_bool: bool, restricted: bool, video_id: str, looper: bool, year_subfolders: bool,
    ...
    if looper:
        ...
        yt_video_counter_label.configure(text=str(v_counter) + " / " + str(total_counter), text_color=COLORS.gray)
